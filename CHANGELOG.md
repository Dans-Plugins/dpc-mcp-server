...
- A `Dockerfile` serving the HTTP transport, for deployment behind the gateway.
  No `npm install` and no `node_modules` in the image — copying `src/` and
  `vendor/` is the whole build — running as the base image's `node` user, with a
  `HEALTHCHECK` that probes `/healthz` using `node`, because a slim image ships
  no `curl`. CI builds the image and drives a session through it, so a broken
  Dockerfile fails here rather than on the box.
